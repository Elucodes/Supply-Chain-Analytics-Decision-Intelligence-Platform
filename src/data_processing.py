...
Loads, validates, cleans and prepares supply chain datasets for analytics.
Includes feature engineering, KPI generation, visual preparation and IoT simulation.
"""
...
VISUAL_DIR = BASE_DIR / "visualisations"
...
class DataProcessor:
    ...
    def __init__(self, db_path=DATABASE_PATH, visual_dir=VISUAL_DIR):
    ...
    def load_and_validate(self,
                          dataco_path=str(DATA_DIR / 'DataCoSupplyChain.csv'),
                          inventory_path=str(DATA_DIR / 'retail_inventory.csv'),
                          logistics_path=str(DATA_DIR / 'smart_logistics.csv')):
...
if __name__ == "__main__":
    p = DataProcessor(db_path=str(DATABASE_PATH), visual_dir=str(VISUAL_DIR))
    ...
    p.close()
    print("\nDATA PROCESSING WORKFLOW COMPLETE (pipeline + continuous IoT mode).")