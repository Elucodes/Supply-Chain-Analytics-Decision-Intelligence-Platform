...
Purpose:
- Interactive Streamlit dashboard for supply chain insights
- Executive KPIs, forecasting, optimisation, risk analysis and real-time visibility

This dashboard presents analytical outputs generated from the supply chain analytics workflow.
"""
...
# IoT DATA: ALWAYS FETCH FRESH DATA
...
db_path = st.sidebar.text_input("SQLite DB Path", "../database/supply_chain.db")
...
with tabs[7]:
    ...
    st.info("""
    **IoT System Validation Methodology**: This tab demonstrates the technical feasibility of IoT-based real-time monitoring. 
    ...
    **Note**: The IoT capability is demonstrated **qualitatively** based on system capability demonstration. 
    ...
    try:
        ...
        if events.empty:
            st.warning("⚠ No IoT events found. Please run the IoT simulation first.")
        else:
            ...
            **IoT Validation Status:** DEMONSTRATED (Qualitative)
        ...
        st.info("Make sure the IoT simulation has been run at least once.")
...
with tabs[8]:
    ...
    try:
        ...
        if df_sup.empty:
            st.warning("⚠ No supplier KPI data found. Run the data processing workflow first.")
        else:
