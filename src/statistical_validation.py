...
Purpose:
- Hypothesis testing across supply chain analytics outputs
- Forecast model validation
- Inventory and logistics optimisation validation
- Risk prediction validation
- Supplier performance statistical analysis
- Real-time visibility validation

This module provides statistical evidence to support data-driven supply chain decisions.
"""
...
def main():
    ...
    if len(error_dfs) >= 2:
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(VIS_DIR,"statistical_validation_forecast_abs_err_box.png"),dpi=300)
            plt.close()
        except Exception:
            pass
    ...
    if not logistics_comp.empty:
        ...
        if {"Baseline","Optimized"}.issubset(pivot.columns):
            ...
            try:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(VIS_DIR,"statistical_validation_logistics_per_vehicle.png"),dpi=300)
                plt.close()
            except Exception:
                pass
            ...
            if not log_co2.empty and {"Baseline","Optimized"}.issubset(pivot_co2.columns):
                try:
                    ...
                    plt.tight_layout()
                    plt.savefig(os.path.join(VIS_DIR,"statistical_validation_logistics_co2.png"),dpi=300)
                    plt.close()
                except Exception:
                    pass
    ...
    try:
        if logistics.empty:
            ...
            print("⚠ No IoT events found — run the IoT data simulation first.")
            results.append({
                "Hypothesis": "H6",
            ...
            if len(merged) >= 3:
                ...
                try:
                    ...
                    plt.grid(True)
                    plt.tight_layout()
                    plt.savefig(os.path.join(VIS_DIR, "statistical_validation_h6_corr.png"), dpi=300)
                    plt.close()
                    print("✔ H6 scatter plot saved: visuals/statistical_validation_h6_corr.png")
                except Exception:
                    pass
            else:
    ...
    to_sql_safe(conn, df_results, "statistical_validation_results")
    try:
        df_results.to_csv(os.path.join(VIS_DIR,"statistical_validation_results.csv"),index=False)
    except Exception:
        pass
    conn.close()
    print("✔ Statistical validation results saved to DB and CSV.")
